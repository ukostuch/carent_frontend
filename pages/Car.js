...
const Car = () => {
  ...
  const carId = router.query.carId; // Assuming you have the carId in the query parameter

  const fetchData = async () => {
    try {
      // Fetch car data from your API or database using the carId
      const response = await fetch(`http://localhost:8080/offer/${carId}`); // Adjust the API endpoint accordingly
    ...
    <div className={styles.car}>
      {carData && (
        <CarCaly
          ...
          color={carData.color}
          drive={carData.drive}
          techCond={carData.techCond}
          passengers={carData.passengers}
          technicalConditionOpis={carData.technicalDescription}
          clientName={carData.clientName}
          cientCountry={carData.cientCountry}
      ...
      <button
        ...
        <img className={styles.image32Icon} alt="" src={carData?.sidePhoto} />
