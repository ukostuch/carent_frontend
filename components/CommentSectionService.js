...
import styles from "./CommentSectionService.module.css";

const CommentSectionService = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8080/current-user', {
      ...
      headers: {
        ...
        // You might need to include additional headers for authentication
      },
    })
  ...
  const handleSend = () => {
    if (!comment || !userName || !userCountry || userId === null) {
      alert('Please fill in all fields before sending the comment.');
      return;
    }
    ...
    fetch('http://localhost:8080/reviews/write', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
...
const CommentSectionService = () => {
  ...
  const apiEndpoint = 'http://localhost:8080/reviews/write';

  const handleSend = () => {
    if (!comment || !userName || !userCountry) {
      alert('Please fill in all fields before sending the comment.');
      return;
    }
...
const CommentSectionService = () => {
  ...
  useEffect(() => {
    ...
    }, 3000); // Ukryj potwierdzenie po 3 sekundach
  ...
  return (
    <div className={styles.commentSection}>
      ...
      <button className={styles.send} onClick={handleSend}>
        <div className={styles.buttonmain}>
          <img className={styles.icons} alt="Send icon" src="/icons.svg" />
          <b className={styles.label}>SEND</b>
        </div>
      </button>
      <textarea
